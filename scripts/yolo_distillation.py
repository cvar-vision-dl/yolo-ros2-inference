#!/usr/bin/env python3
"""
YOLOv11 Knowledge Distillation Training Script with Augmentations, Multi-Scale Training, and TensorBoard
Usage: python train_yolov11_distillation.py --teacher yolo11x.pt --student yolo11n.pt --data path/to/dataset.yaml --epochs 100
"""
...
def parse_arguments():
    """Parse command line arguments for YOLO knowledge distillation training."""
    parser = argparse.ArgumentParser(
        description="Train YOLOv11 with Knowledge Distillation, augmentations, multi-scale training and TensorBoard logging",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to teacher model (e.g., yolo11x.pt, yolo11l.pt, or custom path)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="yolo11n.pt",
        help="Student model to train (yolo11n.pt, yolo11s.pt, yolo11m.pt, etc.)"
    )
    ...
    parser.add_argument(
        ...
        default="cwd",
        choices=["cwd", "dfl", "kd"],
        help="Distillation loss type: 'cwd' (Channel-wise Distillation), 'dfl' (Distribution Focal Loss), 'kd' (Knowledge Distillation)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Distillation loss weight (alpha parameter)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=4.0,
        help="Temperature for distillation softmax (higher = softer targets)"
    )
    ...
    parser.add_argument(
        "--pretrained",
        action="store_true",
        help="Use pretrained weights for student model"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of layers to freeze in student model (0 = no freezing)"
    )
...
def validate_arguments(args):
    ...
    if not os.path.exists(args.teacher):
        ...
        if args.teacher not in valid_models:
            print(f"Warning: Teacher model {args.teacher} not found locally")
            print("Make sure the model path is correct or it will be downloaded automatically")

    # Validate student model
    valid_models = ["yolo11n.pt", "yolo11s.pt", "yolo11m.pt", "yolo11l.pt", "yolo11x.pt"]
    if args.student not in valid_models and not os.path.exists(args.student):
        print(f"Warning: Student model {args.student} not in standard models {valid_models}")
        print("Make sure the model path is correct or it will be downloaded automatically")
    ...
    if args.distill_alpha <= 0:
        raise ValueError("Distillation alpha must be positive")
...
def main():
    ...
    print("=" * 70)
    print("YOLOv11 Knowledge Distillation Training Configuration")
    print("=" * 70)
    print(f"Teacher Model: {args.teacher}")
    print(f"Student Model: {args.student}")
    print(f"Distillation Loss: {args.distillation_loss}")
    print(f"Distillation Alpha: {args.distill_alpha}")
    print(f"Distillation Temperature: {args.distill_temperature}")
    ...
    try:
        ...
        teacher_task = detect_task_type(teacher_model)
        print(f"Teacher model task type: {teacher_task}")
    ...
    try:
        ...
        student_model = YOLO(args.student)
        print(f"Student model loaded successfully")

        # Get student model info
        student_task = detect_task_type(student_model)
        print(f"Student model task type: {student_task}")

        # Verify task compatibility
        if teacher_task != student_task:
            print(f"Warning: Teacher ({teacher_task}) and student ({student_task}) have different task types!")
    ...
    train_args = {
        ...
        "teacher": teacher_model.model,  # Pass the teacher model
                            ...
                            try:
                                ...
                                with open(weights_dir / f"best_img{scale}.txt", "w") as f:
                                    f.write(
                                        ...
                                        f"distillation_loss={args.distillation_loss}\n"
                                        f"from={ckpt_path}\n"
                                    )
    ...
    student_model.add_callback("on_train_epoch_end", _multi_scale_validation_and_log)
    ...
    try:
        ...
        print(f"Best student weights saved to: {student_model.trainer.best}")
        print(f"Last student weights saved to: {student_model.trainer.last}")

        # Print summary of all best models
        if val_scales and any(best_models[scale]["score"] > -1 for scale in val_scales):
            print("\nBest distilled models at different scales:")
            weights_dir = Path(student_model.trainer.save_dir) / "weights"
            for scale in val_scales:
                if best_models[scale]["score"] > -1:
        ...
        print(f"\nError during distillation training: {e}")
        if tb_writer["writer"]:
            tb_writer["writer"].close()
        return 1
